...
# Unified AgentQnA Management Script
# This script provides a unified interface for all AgentQnA operations (TGI and vLLM)
...
check_requirements() {
    ...
    if [[ ${#missing_files[@]} -gt 0 ]]; then
        ...
        print_error "Please run this script from the AgentQnA/docker_compose/amd/gpu/rocm/ directory"
        exit 1
    fi
}
...
detect_services() {
    ...
    if [ "$TGI_RUNNING" = "no" ] && [ "$VLLM_RUNNING" = "no" ]; then
        print_warning "No AgentQnA services are currently running"
    fi
}

# Function to setup TGI environment
setup_tgi_environment() {
    print_header "Setting up AgentQnA TGI Environment"
    check_docker
    check_requirements
...
setup_vllm_environment() {
    print_header "Setting up AgentQnA vLLM Environment"
    check_docker
    check_requirements
...
setup_lightweight() {
    print_header "Setting up Lightweight AgentQnA Environment"
    check_docker
    check_requirements
...
start_tgi_services() {
    print_header "Starting AgentQnA TGI Services"
    check_docker
    check_requirements
...
stop_tgi_services() {
    print_header "Stopping AgentQnA TGI Services"
    check_docker
...
stop_vllm_services() {
    print_header "Stopping AgentQnA vLLM Services"
    check_docker
...
restart_tgi_services() {
    print_header "Restarting AgentQnA TGI Services"
    stop_tgi_services
    sleep 2
    start_tgi_services
}
...
restart_vllm_services() {
    print_header "Restarting AgentQnA vLLM Services"
    stop_vllm_services
    sleep 2
    start_vllm_services
}
...
run_tgi_eval() {
    ...
    if ! docker ps --format "table {{.Names}}" | grep -q "agentqna-tgi-service"; then
    ...
    python evals/benchmark/agentqna_simple_eval.py \
        --service-url http://localhost:8889 \
        --output $EVAL_RESULTS_DIR/agentqna_tgi_eval.json
    
    print_status "TGI evaluation completed!"
    print_status "Results saved to: $EVAL_RESULTS_DIR/agentqna_tgi_eval.json"
...
show_tgi_results() {
    ...
    if [ -f "$EVAL_RESULTS_DIR/agentqna_tgi_eval.json" ]; then
        # Extract and display key metrics
        python3 -c "
import json
try:
    with open('$EVAL_RESULTS_DIR/agentqna_tgi_eval.json', 'r') as f:
        data = json.load(f)
    ...
    if 'error' in summary:
        ...
        print(f'📊 Full results: $EVAL_RESULTS_DIR/agentqna_tgi_eval.json')
...
run_vllm_eval() {
    ...
    python evals/benchmark/agentqna_simple_eval.py \
        ...
        --output $EVAL_RESULTS_DIR/agentqna_vllm_eval.json
    
    print_status "vLLM evaluation completed!"
    print_status "Results saved to: $EVAL_RESULTS_DIR/agentqna_vllm_eval.json"
...
show_vllm_results() {
    ...
    if [ -f "$EVAL_RESULTS_DIR/agentqna_vllm_eval.json" ]; then
        # Extract and display key metrics
        python3 -c "
import json
try:
    with open('$EVAL_RESULTS_DIR/agentqna_vllm_eval.json', 'r') as f:
        data = json.load(f)
    ...
    if 'error' in summary:
        ...
        print(f'📊 Full results: $EVAL_RESULTS_DIR/agentqna_vllm_eval.json')
...
run_comparison_eval() {
    ...
    if [ "$TGI_RUNNING" = "yes" ]; then
        ...
        python evals/benchmark/agentqna_simple_eval.py \
            --service-url http://localhost:8889 \
            --output $EVAL_RESULTS_DIR/agentqna_tgi_comparison.json
        print_status "TGI evaluation completed!"
    fi
    ...
    if [ "$VLLM_RUNNING" = "yes" ]; then
        ...
        python evals/benchmark/agentqna_simple_eval.py \
            ...
            --output $EVAL_RESULTS_DIR/agentqna_vllm_comparison.json
        print_status "vLLM evaluation completed!"
    fi
...
show_comparison_results() {
    ...
    if [ -f "$EVAL_RESULTS_DIR/agentqna_tgi_comparison.json" ] && [ -f "$EVAL_RESULTS_DIR/agentqna_vllm_comparison.json" ]; then
...
import json
try:
    with open('$EVAL_RESULTS_DIR/agentqna_tgi_comparison.json', 'r') as f:
        data = json.load(f)
...
import json
try:
    with open('$EVAL_RESULTS_DIR/agentqna_vllm_comparison.json', 'r') as f:
        data = json.load(f)
...
"
        ...
        print_status "  TGI: $EVAL_RESULTS_DIR/agentqna_tgi_comparison.json"
        print_status "  vLLM: $EVAL_RESULTS_DIR/agentqna_vllm_comparison.json"
        
    elif [ -f "$EVAL_RESULTS_DIR/agentqna_tgi_comparison.json" ]; then
        print_status "Only TGI results available:"
        show_tgi_results
    elif [ -f "$EVAL_RESULTS_DIR/agentqna_vllm_comparison.json" ]; then
...
run_tgi_benchmark() {
    ...
    # Check if TGI services are running
    if ! docker ps --format "{{.Names}}" | grep -q "agentqna-tgi-service"; then
        print_error "TGI services are not running. Please start them first with: $0 start-tgi"
        exit 1
    fi
...
test_cases:
  agentqna:
    e2e:
      service_name: "agentqna-tgi"
      run_test: true
      service_list: ["agentqna-tgi-service"]
      dataset: "default"
      prompts: null
      max_output: 512
      max_new_tokens: 512
...
test_cases:
  agentqna:
    e2e:
      service_name: "agentqna-vllm"
      run_test: true
      service_list: ["agentqna-vllm-service"]
      dataset: "default"
      prompts: null
      max_output: 512
      max_new_tokens: 512
...
show_help() {
    print_header "Unified AgentQnA Management Script Help"
    echo "Usage: $0 [OPTION]"
    echo ""
...
show_menu() {
    while true; do
        print_header "Unified AgentQnA Management Menu"
        echo "Environment Setup:"
